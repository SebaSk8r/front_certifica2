  ...
  Papa.parse(file.value, {
    ...
    complete: async (results) => {
      ...
      for (const result of results.data) {
        ...
        const ot_apertura_date = new Date(
          `${year_a}-${month_a}-${day_a} ${result.OT_APERTURA_HORA}`
        );
        const ot_cierre_date = new Date(
          `${year_c}-${month_c}-${day_c} ${result.OT_CIERRE_HORA}`
        );
        //Ingreso de repuestos
        let curr_repuestos = [];
        if (documentos.has(result.OT_NUMERO.replace(".", "")))
          curr_repuestos = documentos.get(
            result.OT_NUMERO.replace(".", "")
          ).repuestos;
        if (result.REPUESTO_CODIGO !== "")
          curr_repuestos.push({
            ...
            repuesto_descripcion: removeAccents(result.REPUESTO_DESCRIPCION)
              .trim()
              .toUpperCase(),
            repuesto_cantidad:
              parseInt(result.REPUESTO_CANTIDAD.replace(".", "")) || 0,
          });
        //Ingreso de insumos
        let curr_insumos = [];
        if (documentos.has(result.OT_NUMERO.replace(".", "")))
          curr_insumos = documentos.get(
            result.OT_NUMERO.replace(".", "")
          ).insumos;
        if (result.INSUMO_CODIGO !== "")
          curr_insumos.push({
            insumo_codigo: result.INSUMO_CODIGO.trim().toUpperCase(),
            insumo_descripcion: removeAccents(result.INSUMO_DESCRIPCION)
              .trim()
              .toUpperCase(),
            insumo_cantidad:
              parseInt(result.INSUMO_CANTIDAD.replace(".", "")) || "",
          });
        documentos.set(result.OT_NUMERO.replace(".", ""), {
          ...
          ot_apertura_fecha: useDateFormat(ot_apertura_date, "DD/MM/YYYY")
            .value,
          ...
          taller_planta: removeAccents(result.TALLER_PLANTA)
            .trim()
            .toUpperCase(),
          km_ejecucion: parseInt(result.KM_EJECUCION.replace(".", "")),
          pauta_ejecutada: removeAccents(result.PAUTA_EJECUTADA)
            .trim()
            .toUpperCase(),
