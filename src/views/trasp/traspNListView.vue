<template>
  ...
  >
    <template v-slot:top>
      <q-btn v-if="!only_view" color="primary" to="/traspN/add" label="Agregar" class="q-mr-sm" />
...
const columns = [
  ...
  },
  {
    ...
    field: (row) =>
      row.estado === 1 && row.resultado.length == 2 && row.resultado.at(1).certifica === true
        ? "Cumple"
        : row.estado === 1 && row.resultado.length == 2 && row.resultado.at(1).certifica === false
        ? "No Cumple"
        : "",
    align: "center",
    sortable: true,
...
const onEdit = (row) => {
  if (
    [1, 3, 4].includes(row.estado)
    //|| (row.user_uid !== uid && !admin)
  ) {
    notify({
      color: "red-6",
      textColor: "white",
