      ...
      <q-card-section>
        ...
        <q-input
          ...
          :rules="[
            ...
            (val) => !bus_has(val) || 'Estado General ya registrado.',
          ]"
          dense
          mask="XXXXXX"
          debounce="500"
...
import { client, terminales } from "@/client";
...
const { add, addp, getestado } = useEstadogStore();
const { getppu } = useInspecpStore();

const { m_estado_generalp_change } = storeToRefs(useEstadogStore());
const { notify } = useQuasar();
const { name, uid } = useUserStore();
const submited = ref(false);
let m_estado_general = new Set();
const bus_has = (val) => m_estado_general.has(val?.toUpperCase());
...
const onUpdate = async () => {
  ...
  notify({
    ...
    textColor: "white",
    icon: icon,
    message: "Dias Restantes para Inspección Tecnica: " + dias_restantes,
    timeout: 2000,
  });
};
...
const onSubmit = () => {
  ...
  addp({
    ...
    values: placa_patente.value.toUpperCase(),
  });
  notify({
    color: "green-6",
    textColor: "white",
...
watch(
  () => m_estado_generalp_change.value,
  async () => {
    m_estado_general = new Set(
      await getestado({
        uuid: `estados_${client}`,
      })
    );
